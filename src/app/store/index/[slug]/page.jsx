...
export default function Page({params}) {
  ...
  const updateStatusIntoReject = (async (orderId) => {
    let responseOrder = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/orders/reject/${orderId}`, {
      method: "GET",
      cache: "no-store",
      headers: {
        Referer: '127.0.0.1:8000',
    ...
    <>
                        ...
                        <td className="px-4 py-2">
                          {value.status !== "FINISHED" ? (
                            <>
                              ...
                              <Button size={"sm"} onPress={() => updateStatusIntoReject(value.id)}
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">Update Receipt Number</ModalHeader>
              <ModalBody>
                <Input
                  endContent={
